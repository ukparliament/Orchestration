...
namespace Functions.Transformation
{
    public static class TransformationPartyMnis
    {
        ...
        {
            telemetryClient.Context.Operation.Name = "TransformationPartyMnis";
        ...
        {
            string fullDataUrl = $"{dataUrl}?$select=Party_Id,Name";
            string xml = null;
            ...
            try
            {
                ...
                XElement key = doc.XPathSelectElement("atom:entry/atom:content/m:properties/d:Party_Id", xmlNamespaceManager);
                telemetryClient.TrackTrace($"Key: {key.Value}", Microsoft.ApplicationInsights.DataContracts.SeverityLevel.Verbose);
                subjectUri = IdRetrieval.GetSubject("Party", "partyMnisId", key.Value, true, telemetryClient);
        ...
        private static IGraph generateExistingGraph(Uri subject)
        {
            ...
            sparql.SetUri("partyUri", subject);
        ...
        private static IGraph generateNewGraph(XDocument doc, IGraph oldGraph, Uri subjectUri, XmlNamespaceManager xmlNamespaceManager)
        {
            ...
            TripleGenerator.GenerateTriple(result, subject, "parl:partyMnisId", doc, "atom:entry/atom:content/m:properties/d:Party_Id", xmlNamespaceManager);
            TripleGenerator.GenerateTriple(result, subject, "parl:partyName", doc, "atom:entry/atom:content/m:properties/d:Name", xmlNamespaceManager);
            return result;
        }
    }
}